...
## Coupled runner search
`node coupled-feasibility.cjs` explores continuous runner/pawl paths within one tooth pitch. Retained paths in coupled-receipt.json reach seating in all four cases, but require reversals, three selected endpoints unload the assumed pad, and the thin case exceeds the previous travel limit. This is geometric feasibility only, not a passive contact or force solution.
